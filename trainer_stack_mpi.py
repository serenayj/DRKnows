...
class GraphModel(nn.Module):
    ...
    def one_iteration(self, task_emb, cui_lists, running_k, context_emb=None, stop_flag=False,prune_thsh=0.8):
        ...
        if len(list(cand_neighbors_mappings.keys())) == 0:
            ...
            if len(cand_neighbors_mappings) > 5000:
            
                if len(cand_neighbors_mappings) > 15000:
                    pruned_tgt_idx, pruned_cand_neighbors_vs = prune_paths(task_emb, 
                                                                       cand_neighbors_vs, 
                                                                       sorted_cand_neighbors_mappings_keys,
                                                                      threshold=0.5)
                else:
                    pruned_tgt_idx, pruned_cand_neighbors_vs = prune_paths(task_emb, 
                                                                       cand_neighbors_vs, 
                                                                       sorted_cand_neighbors_mappings_keys,
                                                                      threshold=prune_thsh)
                #print("after pruning",len(pruned_tgt_idx))
                remove_tgt_list = list(set(sorted_cand_neighbors_mappings_keys).difference(set(pruned_tgt_idx)))
                #print("removed tgt list ", len(remove_tgt_list))
                cand_neighbors_mappings = {v:k for k,v in enumerate(pruned_tgt_idx)} 
                cand_neighbors_vs = torch.from_numpy(pruned_cand_neighbors_vs) 
                candidate_paths_df = candidate_paths_df[~candidate_paths_df['Tgt'].isin(remove_tgt_list)] # new paths dataframe  
                candidate_paths_df = candidate_paths_df.reset_index(drop=True)
                self.candidate_paths_df = candidate_paths_df 
...
class Trainer(nn.Module):
    ...
    def train(self, train_data, dev_data, lr_scheduler): 
        ...
        update_step = 4
        evaluate_step = int(len(train_data) / 2) # save two evaluation per epoch  
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="SNOMED KG CONTRASTIVE LEARNING + Pruning Diagnosis Generation Config")
    ...
    parser.add_argument('--train_data',dest="train_data",type=str,default="./data/mimic_k2_apr3_train.json")
    parser.add_argument('--dev_data',dest="dev_data",type=str,default="./data/mimic_k2_apr3_dev.json")
    ...
    all_edge_mappings = {k: v for v, k in enumerate(sorted(list(set(edge_labels))))} # fix the order of edge_labels in order to reproduce same result every time
    len(all_edge_mappings)
    node_labels = list(g.nodes())

    exp_name = "k"+str(nums_of_hops)+"_N"+str(top_n)+"_H"+str(nums_of_head)+"_CL"+"_V4_"+distance_metric+"wBERT"
    ...
    val_loader = DataLoader(val_dataset,collate_fn=collate_fn, batch_size=1, shuffle=False) 
